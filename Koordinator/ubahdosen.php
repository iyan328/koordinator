...
<?php
	...
	if(isset($_GET['kode'])){
		...
		echo "<script>alert('Dosen Belum Dipilih');document.location='akundosenp.php'</script>";
	}
	$sql = "SELECT * FROM p_poltek WHERE username='$kode'";
	$kueri = mysql_query($sql);
	$data = mysql_fetch_array($kueri);
	$user= $data['username'];
	$sql1 = "SELECT * FROM akun WHERE username='$user'";
...
<body>
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">
                            Ubah Akun <small><?php echo $nama1?></small>
                        </h1>
                        <ol class="breadcrumb">
                            <li>
							...
							<li>
                                <i class="fa fa-user"></i> <a href="akundosenp.php"> Akun Dosen Pembimbing </a>
                            </li>
							<li class="active">
                                <i class="fa fa-user"></i> Ubah Akun
                            </li>
                        </ol>
                    </div>
                </div>
				...
				<div class="col-lg-4">
                    <form role="form" action="" method="post">
							...
							<div class="form-group">
								<label>Nik</label>
								<input name="nik" type="text" class="form-control" value="<?php echo $nik?>" readonly>
								<!-- <p class="help-block">Example block-level help text here.</p> -->
                            </div>
							<div class="form-group">
                                <label>Username</label>
                                <input name="user" class="form-control" type="text" value="<?php echo $user1?>" required>
                            </div>
							<div class="form-group">
								<label>Password</label>
								<input name="pass" type="password" class="form-control" value="<?php echo $pass?>" required>
								<!-- <p class="help-block">Example block-level help text here.</p> -->
                            </div>
	...
	$user1= $data1['username'];
			...
			$username = $_POST['user'];
			$password = $_POST['pass'];
			$sql = "UPDATE akun set username='$username', password='$password' where username='$user1'";
			$kueri = mysql_query($sql);
			if ($kueri){
				echo "<script> document.location='akundosenp.php'</script>";
			}
			else {
				echo "<script> alert('Data gagal diubah');document.location='ubahdosen.php'</script>";
				echo mysql_error();
			}
		}
		?>		
